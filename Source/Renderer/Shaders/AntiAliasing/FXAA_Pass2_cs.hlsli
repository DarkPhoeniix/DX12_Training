...
{
    Texture2D<float> Luma           = ResourceDescriptorHeap[PassCB.LumaTextureIndex];
    ByteAddressBuffer WorkQueue     = ResourceDescriptorHeap[PassCB.WorkQueueBufferIndex];
    Buffer<uint> ColorQueue         = ResourceDescriptorHeap[PassCB.ColorQueueBufferIndex];

    RWTexture2D<float3> DstColor    = ResourceDescriptorHeap[PassCB.OutputTextureIndex];
    
#ifdef VERTICAL_ORIENTATION
    uint ItemIdx = PassCB.LastQueueIndex - DTid.x;
#else
    uint ItemIdx = DTid.x;
#endif
    uint WorkHeader = WorkQueue.Load(ItemIdx * 4);
    uint2 ST = PassCB.StartPixel + (uint2(WorkHeader >> 8, WorkHeader >> 20) & 0xFFF);
    ...
    float2 StartUV = (ST + float2(GradientDir, 0.5f)) * PassCB.RcpTextureSize;
#else
    float NextLuma = Luma[ST + int2(0, GradientDir * 2 - 1)];
    float2 StartUV = (ST + float2(0.5, GradientDir)) * PassCB.RcpTextureSize;
#endif
    ...
    for (uint iter = 0; iter < NUM_SAMPLES; ++iter)
    {
...
#ifdef VERTICAL_ORIENTATION
        float2 NegUV = StartUV - float2(0, PassCB.RcpTextureSize.y) * Distance;
        float2 PosUV = StartUV + float2(0, PassCB.RcpTextureSize.y) * Distance;
#else
        float2 NegUV = StartUV - float2(PassCB.RcpTextureSize.x, 0) * Distance;
        float2 PosUV = StartUV + float2(PassCB.RcpTextureSize.x, 0) * Distance;
#endif

        // Check for a negative endpoint
        float NegGrad = Luma.SampleLevel(LinearClampSampler, NegUV, 0) - CenterLuma;
        ...
        float PosGrad = Luma.SampleLevel(LinearClampSampler, PosUV, 0) - CenterLuma;

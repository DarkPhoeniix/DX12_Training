...
{
    swapChain->GetBuffer(Index, IID_PPV_ARGS(&_swapChainTexture));
    ...
    D3D12_RESOURCE_DESC resDesc = _swapChainTexture->GetDesc();
    resDesc.Flags |= D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;

    D3D12_RESOURCE_ALLOCATION_INFO asdas =  device->GetResourceAllocationInfo(0, 1, &resDesc);

    _targetTexture = Helper::CreateBuffers(device, resDesc, &clearValueTexTarget, D3D12_RESOURCE_STATE_COPY_SOURCE);
    ...
    _depthTexture = Helper::CreateBuffers(device, resDesc, &clearValue, D3D12_RESOURCE_STATE_DEPTH_WRITE);
...

    {
        ...
        device->CreateRenderTargetView(_targetTexture.Get(), &renderTargetdesc, _targetHeap->GetCPUDescriptorHandleForHeapStart());
    }

    {
        ...
        device->CreateDepthStencilView(_depthTexture.Get(), &depthStencilDesc, _depthHeap->GetCPUDescriptorHandleForHeapStart());
    }
}
